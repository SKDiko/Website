...
if (isset($_GET['allergy_checked']) && isset($_GET['med_interaction_checked']) && isset($_GET['med_contraindication_checked']) && !(isset($_POST['btnClear']))) {
	...
	if($_GET['allergy_checked'] == 'no') {
		...
		$_SESSION['load_med_id'] = mysqli_real_escape_string($conn, substr($_POST['medication'], 0, 7));
		$_SESSION['load_med_name'] = mysqli_real_escape_string($conn, substr($_POST['medication'], 10));
		...
		$_SESSION['load_pat_id'] = mysqli_real_escape_string($conn, $_SESSION['med_history_id_num']);
	...
	if($_SESSION['load_pat_id'] == ""){
 		...
 		echo("<script>window.location = '../doctor-home.php?load-prescription=error';</script>");
 		mysqli_close($conn);
		exit();
	}

	if($_SESSION['load_med_id'] == ""){
 		echo("<script>alert('Load Prescription Unsuccessful: Medication field is empty.')</script>");
	...
	if($_GET['allergy_checked'] == 'no') {
		...
		if($resultCheck1 > 0){
			...
			if ($msg != ""){
				echo "
				<script>
					if (confirm('Medication Allergic Reaction: This patient have [".substr($msg, 0, -2)."] allergic reaction, ".$_SESSION['load_med_name']." medication is not recommended for this patient. Do you want to ignore this alert?')) {
						var reason;
	...
	if($_GET['med_interaction_checked'] == 'no') {
		...
		if($resultCheck1 > 0){
			...
			while($row1 = mysqli_fetch_assoc($result1)){
				...
				$sql2 = "SELECT * FROM PRESCRIPTION WHERE PATIENT_ID ='".$_SESSION['load_pat_id']."' AND NOT CURRENT_REPEATS = '0' AND NOT STATUS ='Unconfirmed'";
				$result2 = mysqli_query($conn, $sql2);
				$resultCheck2 = mysqli_num_rows($result2);
				if($resultCheck2 > 0){
	...
	$sql1 = "INSERT INTO PRESCRIPTION (PRESCRIPTION_ID, MED_ID, QUANTITY, ORIGINAL_REPEATS, CURRENT_REPEATS, STATUS, INSTRUCTIONS, PRESCRIPTION_DATE, DOCTOR_ID, PATIENT_ID)
	VALUES (
	...
	if (mysqli_query($conn, $sql1)) {
		...
		echo("<script>alert('Add Prescription Successful')</script>");
	 	echo("<script>window.location = '../doctor-load-prescriptions.php?add-prescription=success';</script>");
	 	mysqli_close($conn);
		exit();
		
	} else {
 		echo("<script>alert('Add Prescription Unsuccessful: Something went wrong, please try again.')</script>");
 		echo("<script>window.location = '../doctor-load-prescriptions.php?add-prescription=error';</script>");
 		mysqli_close($conn);
		exit(); 
	}
	...
	session_unset();
	session_destroy();
	header("Location: ../index.php?add-prescription=error");
	mysqli_close($conn);
	exit(); 
}