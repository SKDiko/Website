	...
	if($stmt = mysqli_prepare($conn, $sql)){
		...
		if ($row = mysqli_fetch_assoc($result)){
			...
			if($tokenCheck == true){
				...
				$sql = "SELECT * FROM PHARMACIST WHERE REGISTRATION_NUM = ? AND EMAIL = ?;";
				if($stmt = mysqli_prepare($conn, $sql)){
					...
					if ($row = mysqli_fetch_assoc($result)){
						...
						if(mysqli_stmt_prepare($stmt, $sql)){
							...
							$sql = "UPDATE PHARMACIST SET PASSWORD = ? WHERE REGISTRATION_NUM = ? AND EMAIL = ?;";
							if(mysqli_stmt_prepare($stmt, $sql)){
								mysqli_stmt_bind_param($stmt, "sss", $confirm_pswd, $userName, $userEmail);
								mysqli_stmt_execute($stmt);
								mysqli_stmt_close($stmt);
								echo("<script>alert('Pharmacist Password Reset Successful')</script>");
