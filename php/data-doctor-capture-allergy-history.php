...
if (isset($_POST['btnSubmit'])) {
	...
	if($pat_id == ""){
		echo("<script>alert('Capture Allergy History Unsuccessful: Patient personal details are empty.')</script>");
 		echo("<script>window.location = '../doctor-home.php?capture-allergy-history=error';</script>");
 		mysqli_close($conn);
		exit();
	}
	...
	if($resultCheck > 0){
		echo("<script>alert('Capture Allergy History Unsuccessful: Allergy: ".substr($allergy_name, 10).", for Patient: ".$pat_id." on the ".$med_date." is already captured.')</script>");
 		echo("<script>window.location = '../doctor-capture-allergy-history.php?capture-allergy-history=error';</script>");
 		mysqli_close($conn);
		exit();
	}
	...
	if (mysqli_query($conn, $sql)) {
 		...
 		echo("<script>window.location = '../doctor-capture-allergy-history.php?capture-allergy-history=success';</script>");
 		mysqli_close($conn);
		exit(); 
	} else {
