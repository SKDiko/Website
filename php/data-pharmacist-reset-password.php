...
if (isset($_POST['btnReset'])) {
	...
	if($stmt = mysqli_prepare($conn, $sql)){
		mysqli_stmt_bind_param($stmt, "s", $GLOBALS['pharm_reset_reg_num']);
		mysqli_stmt_execute($stmt);
	}else{
		echo("<script>alert('Reset Password Unsuccessful: Something went wrong, Please try again.')</script>");
	 	echo("<script>window.location = '../pharmacist-reset-password.php?reset-password=error';</script>");
	 	mysqli_close($conn);
		exit();
	}
	...
	if($stmt = mysqli_prepare($conn, $sql)){
		...
		mysqli_stmt_bind_param($stmt, "sssss", $GLOBALS['pharm_reset_reg_num'], $GLOBALS['pharm_reset_email'], $selector, $hashedToken, $expires);
		mysqli_stmt_execute($stmt);
	}else{
		echo("<script>alert('Reset Password Unsuccessful: Something went wrong, Please try again.')</script>");
	 	echo("<script>window.location = '../pharmacist-reset-password.php?reset-password=error';</script>");
	...
	//send email
	$to = $GLOBALS['pharm_reset_email'];
	
	$subject = 'Reset your password from E-Prescribing System';
	$message = '<p>Dear '.$GLOBALS['pharm_reset_name'].' '.$GLOBALS['pharm_reset_surname'].' ('.$GLOBALS['pharm_reset_reg_num'].')';
	...
	$header .= "MIME-Version: 1.0 \r\n";  
   	...
   	if( $result == true){ 
   	  	...
   	  	echo("<script>window.location = '../pharmacist-login.php?email-sent=success';</script>");
		...
		exit(); 
   	  	...
   	  	echo("<script>window.location = '../pharmacist-reset-password.php?email-sent=error';</script>");
   	  	mysqli_stmt_close($stmt);
		mysqli_close($conn);
		exit();    
   }
}else{
	header("Location: ../pharmacist-reset-password.php?reset-password=error");
	exit();
}