...
if (isset($_POST['btnSubmit'])) {
	...
	$pat_id = mysqli_real_escape_string($conn, $_SESSION['first_visit_id_num']);
	...
	if($pat_id == ""){
		echo("<script>alert('Capture Medical History Unsuccessful: Patient personal details are empty.')</script>");
 		echo("<script>window.location = '../doctor-first-medical-history.php?capture-medical-history=error';</script>");
 		mysqli_close($conn);
		exit();
	}
	...
	if (mysqli_query($conn, $sql)) {
		...
		$_SESSION['first-visit-email'] = "";
 		...
 		echo("<script>window.location = '../doctor-first-medical-history.php?capture-medical-history=success';</script>");
 		mysqli_close($conn);
		exit(); 
	} else {
	...
	$_SESSION['first-visit-contact-num'] = "";
	$_SESSION['first-visit-email'] = "";
	echo("<script>window.location = '../doctor-first-medical-history.php?clear-medical-history=success';</script>");
}else{
	session_unset();
	session_destroy();
