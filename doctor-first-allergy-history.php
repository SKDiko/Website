...
<?php include 'php/inc-doctor-first-allergy-search-patient-data.php' ?>

<!DOCTYPE html>
<html lang="en-US">
<head>
<title>First Visit Allergy History | E-Prescribing</title>
...
<form class="form-content" action="php/data-doctor-first-allergy-history-search-patient.php" method="POST" enctype="multipart/form-data" autocomplete="on">
	<h2>First Visit Allergy History</h2>
...
<form class="form-content" action="php/data-doctor-first-allergy-history.php" method="POST" enctype="multipart/form-data" autocomplete="on">

	<fieldset>
		<legend>Patient Personal Details</legend>
		<?php echo '
		<label for="pat_id">ID Number:</label><br>
		<input type="text" name="pat_id" id="pat_id" value="'.$_SESSION['first_allergy_id_num'].'" disabled><br>
		
		<label for="pat_name">Patient Name:</label><br>
		<input type="text" name="pat_name" id="pat_name" value="'.$_SESSION['first_allergy_name'].' '.$_SESSION['first_allergy_surname'].'" disabled><br>

		<label for="pat_age">Patient Age:</label><br>
		<input type="text" name="pat_age" id="pat_age" value="'.$_SESSION['first_allergy_age'].'" disabled><br>

		<label for="pat_gender">Gender:</label><br>
		<input type="text" name="pat_gender" id="pat_gender" value="'.$_SESSION['first_allergy_gender'].'" disabled><br>
...
'.$_SESSION['first_allergy_code'].'
		...
		<input type="text" name="pat_contact_num" id="pat_contact_num" value="'.$_SESSION['first_allergy_contact_num'].'" disabled><br>

		<label for="email">Email:</label><br>
		<input type="email" name="email" value="'.$_SESSION['first_allergy_email'].'" id="email" disabled><br>
	</fieldset>
	';?>

	<fieldset>
		<legend>Allergy History</legend>
		<?php include 'php/inc-get-patient-allergy.php'; ?>
...
<script src="js/allergy-required-fields.js"></script>
